...
var _ = Describe("CNIGenie", func() {
	...
	Describe("Add calico networking for Pod", func() {
		...
		Context("using cni-genie for configuring calico CNI", func() {
			...
			It("should succeed calico networking for pod", func() {
				...
				_, err := clientset.CoreV1().Pods(TEST_NAMESPACE).Create(context.Background(), &v1.Pod{
				...
				pod, err := clientset.CoreV1().Pods(TEST_NAMESPACE).Get(context.Background(), name, metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
				glog.Info("pod status =", string(pod.Status.Phase))
				Expect(string(pod.Status.Phase)).To(Equal("Running"))

				By("Pod was in Running state... Time to delete the calico pod now...")
				err = clientset.CoreV1().Pods(TEST_NAMESPACE).Delete(context.Background(), name, metav1.DeleteOptions{})
				Expect(err).NotTo(HaveOccurred())
				By("Waiting 5 seconds")
				time.Sleep(time.Duration(5 * time.Second))
				By("Check for calico pod deletion")
				_, err = clientset.CoreV1().Pods(TEST_NAMESPACE).Get(context.Background(), name, metav1.GetOptions{})
	...
	Describe("To create logical network crd objects ", func() {
		It("should succeed crd creation", func() {
			...
			_, err = apiextensionsclient.ApiextensionsV1beta1().CustomResourceDefinitions().Create(context.Background(), crd, metav1.CreateOptions{})
			Expect(err).NotTo(HaveOccurred())
	...
	Describe("To create NetworkAttachmentDefinition CRD ", func() {
		...
		It("To create NetworkAttachmentDefinition object flannel", func() {
			...
			time.Sleep(time.Duration(10 * time.Second))
			_, err = netattachclient.K8sCniCncfIoV1().NetworkAttachmentDefinitions(TEST_NAMESPACE).Create(context.Background(), &netattachobj, metav1.CreateOptions{})
			Expect(err).NotTo(HaveOccurred())
			Expect("Success").To(Equal("Success"))
		})
		...
		It("To create NetworkAttachmentDefinition object weave", func() {
			...
			time.Sleep(time.Duration(5 * time.Second))
			_, err = netattachclient.K8sCniCncfIoV1().NetworkAttachmentDefinitions(CRD_TEST_NAMESPACE).Create(context.Background(), &netattachobj, metav1.CreateOptions{})
			Expect(err).NotTo(HaveOccurred())
			Expect("Success").To(Equal("Success"))
		})
...
var _ = AfterSuite(func() {
	...
	err := apiextensionsclient.ApiextensionsV1beta1().CustomResourceDefinitions().Delete(context.Background(), NetAttachDef, metav1.DeleteOptions{})
	if err != nil {
		panic(err)
	}
	//delete namespace network
	err = clientset.CoreV1().Namespaces().Delete(context.Background(), CRD_TEST_NAMESPACE, metav1.DeleteOptions{})
	//delete crd logicalnetworks
	err = apiextensionsclient.ApiextensionsV1beta1().CustomResourceDefinitions().Delete(context.Background(), NetworkCRD, metav1.DeleteOptions{})
	if err != nil {
		panic(err)
	}
	err = clientset.CoreV1().Pods("default").DeleteCollection(context.Background(), metav1.DeleteOptions{}, metav1.ListOptions{})
	// Delete namespace
	err = clientset.CoreV1().Namespaces().Delete(context.Background(), TEST_NAMESPACE, metav1.DeleteOptions{})
	// Delete all pods
	err = clientset.CoreV1().Pods(TEST_NAMESPACE).DeleteCollection(context.Background(), metav1.DeleteOptions{}, metav1.ListOptions{})
	if err != nil {
		panic(err)
	}
...
func createNamespace(clientset *kubernetes.Clientset) {
	ns, err := clientset.CoreV1().Namespaces().Create(context.Background(), &v1.Namespace{
	...
	By("Waiting 5 seconds")
	time.Sleep(time.Duration(5 * time.Second))
	ns, err = clientset.CoreV1().Namespaces().Get(context.Background(), TEST_NAMESPACE, metav1.GetOptions{})
	Expect(err).ShouldNot(HaveOccurred())
	Expect(ns.Name).To(Equal(TEST_NAMESPACE))

	networkns, err := clientset.CoreV1().Namespaces().Create(context.Background(), &v1.Namespace{
	...
	By("Waiting 5 seconds")
	time.Sleep(time.Duration(5 * time.Second))
	networkns, err = clientset.CoreV1().Namespaces().Get(context.Background(), CRD_TEST_NAMESPACE, metav1.GetOptions{})
	Expect(err).ShouldNot(HaveOccurred())
